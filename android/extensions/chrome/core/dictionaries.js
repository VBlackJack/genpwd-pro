...
async function loadDictionarySource(config) {
  ...
  for (const candidate of candidatePaths) {
    try {
      ...
      safeLog(`Local dictionary read ${config.name} from ${candidate}`);
...
// Internal dictionary system state
const dictionaries = {
  current: 'french',
  cache: new Map(),
  status: new Map(),
...
export async function loadDictionary(dictKey) {
  ...
  const loadPromise = (async () => {
    ...
    updateDictionaryInfo(dictKey, words.length, data.metadata);
      ...
      safeLog(`[Dictionary] Error loading ${dictKey}: ${error.message}`);
      updateDictionaryStatus(dictKey, 'error');
      updateDictionaryInfo(dictKey, 0, null, error.message);

      // Fallback to the built-in French dictionary
      if (dictKey !== 'french') {
        safeLog('[Dictionary] Falling back to built-in French dictionary');
...
export function initializeDictionaries() {
  // Initialize the French fallback
  updateDictionaryStatus('french', 'fallback');
  updateDictionaryInfo('french', FALLBACK_DICTIONARY.length,
    { version: 'v1.0', source: 'built-in' });
  
  safeLog('[Dictionary] Dictionary system initialized');
}
